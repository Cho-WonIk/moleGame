...
void AUX_RGBImageRec::convertBGRtoRGB()
{
	...
	std::byte Temp;  // not quick but it works  
...
bool AUX_RGBImageRec::loadFile(const char* Filename)
{
	...
	data = new std::byte[BitmapLength];					// Get windows to do the dirty work of converting the BMP into the format needed by OpenGL  
